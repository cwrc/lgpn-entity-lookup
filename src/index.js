...
const getEntitySourceURI = (queryString) => {
  ...
  return `https://lookup.services.cwrc.ca/lgpn2/cgi-bin/lgpn_search.cgi?name=${encodedQuery};style=json`;
};
...
const callLGPN = async (url) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Something wrong with the call to LGPN, possibly a problem with the network or the server. HTTP error: ${response.status}`
    );
  }
